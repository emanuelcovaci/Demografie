        ...
        if entry_active(entry, active_entries):
            text += '<li class="active">\
                        <ul class="collapsible collapsible-accordion">\
                            <li>\
                                <a style="padding-left: 16px;" href="' + (unicode(reverse_lazy(entry["url_name"])) if not has_children else '#') + '" class="collapsible-header active"><i style="margin-left:' + str(depth * 2) + '0px" class="material-icons">' + entry["icon"] + '</i>' + entry["verbose_name"] + '</a>\
            ...
            text += '<li>\
                        <ul class="collapsible collapsible-accordion">\
                            <li>\
                                <a style="padding-left: 16px;"  href="' + (unicode(reverse_lazy(entry["url_name"])) if not has_children else '#') + '" class="collapsible-header"><i style="margin-left:' + str(depth * 2) + '0px" class="material-icons">' + entry["icon"] + '</i>' + entry["verbose_name"] + '</a>\
...
def generate_list(request):
    ...
    for entry in DASH_ENTRIES.itervalues():
        ...
        if entry_active(entry, active_entries):
            text += '<li class="active">\
                        <a href="' + (unicode(reverse_lazy(entry["url_name"])) if not has_children else '#') + '" class="collapsible-header active"><i class="material-icons">' + entry["icon"] + '</i>' + entry["verbose_name"] + '</a>\
                        <div class="collapsible-body" style="display:block">\
                            <ul>' + \
            ...
            text += '<li>\
                        <a href="' + (unicode(reverse_lazy(entry["url_name"])) if not has_children else '#') + '" class="collapsible-header"><i class="material-icons">' + entry["icon"] + '</i>' + entry["verbose_name"] + '</a>\
                        <div class="collapsible-body">\
                            <ul>' + \
